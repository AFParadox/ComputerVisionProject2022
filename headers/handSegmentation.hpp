...
cv::Mat segmentHandsWatershed(cv::Mat img, std::vector<cv::Rect2i> bboxes);
...
void setHandMarkersWithGraphSegm(cv::Mat hand, cv::Rect2i centralKernel, cv::Mat * markers);    // compute hand markers using graph Segmentation and "central kernel"
...
void saveHandIstances(std::string name, cv::Mat img, std::vector<cv::Rect2i> bboxes, std::string destDir);    // save the contents of each bounding box detected in localization task in a image
