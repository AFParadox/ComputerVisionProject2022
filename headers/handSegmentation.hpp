...
cv::Rect enlargeBBox(cv::Rect bbox, int rows, int cols, float scale);   // return larger bounding box(segmention was tested over the result of a much weaker network compared to the current one)
